    ...
    return (
      <div className="space-y-1.5">
        ...
        <select
          ...
          } rounded-lg px-3.5 py-2.5 text-sm text-slate-100 focus:outline-none focus:ring-2 transition-all duration-150 ${className}`}
          {...props}
        >
          {placeholder && (
            <option value="" disabled className="bg-[#0e1422] text-slate-500">
              {placeholder}
            </option>
          )}
          {options.map((opt) => (
